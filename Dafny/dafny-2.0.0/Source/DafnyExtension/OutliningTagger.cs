...
namespace DafnyLanguage
{
  ...
  /// <summary>
  /// Translate Dafny1ResolverTag's into IOutliningRegionTag's
  /// </summary>
  internal sealed class OutliningTagger : ITagger<IOutliningRegionTag>
  {
    ITextBuffer _buffer;
    ITextSnapshot _snapshot;  // the most recent snapshot of _buffer that we have been informed about
    Dafny1.Program _program;  // the program parsed from _snapshot
    ...
    bool ComputeOutliningRegions(Dafny1.Program program, ITextSnapshot snapshot) {
      ...
      foreach (var module in program.Modules()) {
        ...
        foreach (Dafny1.TopLevelDecl d in module.TopLevelDecls) {
          if (!HasBodyTokens(d) && !(d is Dafny1.ClassDecl)) {
            continue;
          }
          if (d is Dafny1.OpaqueTypeDecl) {
            OutliningRegion.Add(newRegions, program, d, "type");
          } else if (d is Dafny1.CoDatatypeDecl) {
            OutliningRegion.Add(newRegions, program, d, "codatatype");
          } else if (d is Dafny1.DatatypeDecl) {
            OutliningRegion.Add(newRegions, program, d, "datatype");
          } else if (d is Dafny1.ModuleDecl) {
            // do nothing here, since the outer loop handles modules
          } else {
            var cl = (Dafny1.ClassDecl)d;
            if (cl.IsDefaultClass) {
              // do nothing
            } else if (cl is Dafny1.IteratorDecl) {
            ...
            foreach (Dafny1.MemberDecl m in cl.Members) {
              if (!HasBodyTokens(m)) {
                continue;
              }
              if (m is Dafny1.Function && ((Dafny1.Function)m).Body != null) {
                var nm =
                  m is Dafny1.InductivePredicate ? "inductive predicate" :
                  m is Dafny1.CoPredicate ? "copredicate" :
                  // m is Dafny1.PrefixPredicate ? "prefix predicate" :  // this won't ever occur here
                  m is Dafny1.Predicate ? "predicate" :
                  "function";
                if (!m.IsGhost) {
                  nm += " method";
                }
                OutliningRegion.Add(newRegions, program, m, nm);
              } else if (m is Dafny1.Method && ((Dafny1.Method)m).Body != null) {
                var nm =
                  m is Dafny1.Constructor ? "constructor" :
                  m is Dafny1.CoLemma ? "colemma" :
                  m is Dafny1.Lemma ? "lemma" :
                  // m is Dafny1.PrefixLemma ? "prefix lemma" :  // this won't ever occur here
                  "method";
                if (m.IsGhost && !(m is Dafny1.CoLemma)) {
                  nm = "ghost " + nm;
                }
                OutliningRegion.Add(newRegions, program, m, nm);
              }
    ...
    static bool HasBodyTokens(Dafny1.Declaration decl) {
    ...
    class OutliningRegion : DafnyRegion
    {
      ...
      private OutliningRegion(Dafny1.INamedRegion decl, string kind) {
